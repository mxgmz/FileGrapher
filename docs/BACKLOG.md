...
- ✅ **Gravity + minimal-motion (S30, PR #18)** — a note's *first* link pulls it beside its kin
  (`placeNearKin` via `nearestFreeCenter`); `canvas_arrange` skips spokes already within
  `arrangeSettleRadius` (24pt) of their slot (re-run on a tidy hub = no-op), nearest-slot pairing. Both
  live as an `extension AppModel` in `MCPServer.swift`. `Tests/CartographerGravityTests` (11 assertions).
- ✅ **Layout-switching (S30, PR #21)** — `canvas_arrange` gains a `layout` arg: `radial`/`grid`/`columns`,
  or `auto` (default) which picks from the link topology (`autoLayout`: hub→most spokes ⇒ radial, chain ⇒
...
- ✅ **The agent gardening loop (S30, PR #23)** — `tools/garden/` (custodian prompt + `run-garden.sh <vault>`):
  wires a headless `claude` agent to the running app's MCP server and runs `canvas_health → tidy LAYOUT →
...
- ✅ **Drag a file in from Finder (S30, PR #17)** — drop `.md`/`.csv`/code files or a folder onto the
  canvas → copied into the vault (collision-safe) + boxed at the drop point; one ⌘Z reverses box + file.
  Canvas-background `.onDrop` + `FinderImport.swift` (`importFiles`/`tImport`).
...
### Epic: Productivity
- ✅ **Search / quick-open (⌘P) (S30, PR #16)** — fuzzy substring palette over all nodes (prefix-first),
  ↑/↓/Enter/Esc; Enter pans-to + selects (reuses `center(on:)`). `QuickOpen.swift`.
- ⬜ Recent vaults list.
